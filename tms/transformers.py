...
__all__ = ['Config', 'HookPoint', 'Embed', 'Unembed', 'PosEmbed', 'LayerNorm', 'Attention', 'MLP', 'TransformerBlock',
           'Transformer', 'make_fourier_basis', 'calculate_key_freqs', 'calculate_key_freqs_vectorised', 'get_indices_for_key_freqs_calculation', 'get_components_of_trig_loss',
...
def get_indices_for_key_freqs_calculation(config: Config):
    listA = [[i]*3 for freq in range(1, config.p // 2) for i in [0, 2*freq-1, 2*freq]]
    listB = [[0, 2*freq-1, 2*freq] for freq in range(1, config.p //2) for idx in range(3)]
    return [listA , listB]

def calculate_key_freqs_vectorised(config: Config, model: Transformer, all_data, labels, fourier_basis, indices):
    cache = {}
    model.remove_all_hooks() # TODO is this line fucky??
    model.cache_all(cache)
    model(all_data)
    neuron_acts = cache['blocks.0.mlp.hook_post'][:, -1].clone()
    cache.clear()
    ...
    mlp_dim = fourier_neuron_acts_square.shape[-1]

    final_result = fourier_neuron_acts_square[indices].pow(2).reshape(config.p // 2 - 1, 3, 3, mlp_dim).sum(dim=1).sum(dim=1).T
    fracs = final_result / fourier_neuron_acts_square.pow(2).sum(dim=0).sum(dim=0).reshape(mlp_dim, 1)
    neuron_frac_explained, neuron_freqs = fracs.max(dim=1)
    neuron_freqs += 1
    key_freqs, neuron_freq_counts = np.unique(helpers.to_numpy(neuron_freqs), return_counts=True)
    return key_freqs

def calculate_key_freqs(config: Config, model: Transformer, all_data, labels, fourier_basis):
    ...
    for ni in range(config.d_mlp):
        ...
        for freq in range(1, config.p//2):
            ...
            last_denom = denominator
            '''
            if denominator == 0:
                print('denom is zero: ', denominator, best_freq, ni, freq, frac_explained, best_frac_explained)
            '''
...
def get_components_of_trig_loss_vectorised(logits, key_freqs, fourier_basis):
    odd_freqs = [2 * freq - 1 for freq in key_freqs]
    even_freqs = [2 * freq for freq in key_freqs]
    
    cosx_cosy_directions = (fourier_basis[odd_freqs, :, None] * fourier_basis[odd_freqs, None, :]).flatten(start_dim=1, end_dim=-1)
    sinx_siny_directions = (fourier_basis[even_freqs, :, None] * fourier_basis[even_freqs, None, :]).flatten(start_dim=1, end_dim=-1)
    cos_xpy_directions = (cosx_cosy_directions - sinx_siny_directions) / np.sqrt(2)
    
    sinx_cosy_directions = (fourier_basis[even_freqs, :, None] * fourier_basis[odd_freqs, None, :]).flatten(start_dim=1, end_dim=-1)
    cosx_siny_directions = (fourier_basis[odd_freqs, :, None] * fourier_basis[even_freqs, None, :]).flatten(start_dim=1, end_dim=-1)
    sin_xpy_directions = (sinx_cosy_directions + cosx_siny_directions)/np.sqrt(2)

    
    cos_vec = (cos_xpy_directions[ : , : , None] @ (cos_xpy_directions[: , None , : ] @ logits))
    sin_vec = (sin_xpy_directions[ : , : , None] @ (sin_xpy_directions[: , None , : ] @ logits))
    
    return cos_vec.sum(dim=0) + sin_vec.sum(dim=0)


def calculate_excluded_loss(config: Config, is_train, is_test, labels, logits, trig_logits):
    new_logits = logits - trig_logits
    return helpers.test_logits(new_logits, bias_correction=False, mode='train', p = config.p,
...
def calculate_trig_loss(config: Config, model, train, logits, key_freqs, fourier_basis, all_data, is_train, is_test, labels, mode='all'):
    ...
    trig_logits = get_components_of_trig_loss_vectorised(logits, key_freqs, fourier_basis)
